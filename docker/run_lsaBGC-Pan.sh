...
if [[ $# -eq 0 || $1 == "-h" || $1 == "--help" || $1 == "-v" || $1 == "--version" ]]; then
        docker pull raufs/lsabgc_pan:latest
        docker run \
        --detach=false \
        --rm \
        --user=$(id -u):$(id -g) \
        raufs/lsabgc_pan:latest \
        ...
        while [[ ! $# -eq 0 ]]; do
                if [[ "$1" == '-g' || "$1" == '--genomes' ]]; then
                        ...
                        shift
                elif [[ "$1" == '-a' || "$1" == '--antismash-results-directory' ]]; then
                        shift 
                        ABS_VALUE=$(get_abs_filename $1)
                        INPUT_DIR=$(basename $ABS_VALUE)
                        INPUT_PARENT_DIR=$(direname $ABS_VALUE)
                        EASY_ARGS+="-a $CONTAINER_INPUT_DIR/$INPUT_DIR/ "
                        DOCKER_VOLUME_ARGS+="--volume $INPUT_PARENT_DIR:$CONTAINER_INPUT_DIR:ro "
                        shift
        ...
        # run workflow
        docker pull raufs/lsabgc_pan:latest
        docker run ${DOCKER_VOLUME_ARGS} --detach=false --rm --user=$(id -u):$(id -g) raufs/lsabgc_pan:latest ${EASY_ARGS}
fi
